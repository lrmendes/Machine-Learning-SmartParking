...
output_graph_png = "OutputGraphs/parking_occupation_from_sensor_minimum_query.png"
plot_year = 2016
# The parking lot have 5500 spots
# plot only time series where available parking spots number is lower than
plot_from_min = 500 #Plot only when 90%+ of parking is filled
outpuv_csv = 'OutputDatasets/parking-klcc-standardized_minimum_query.csv'
...
data = data[data['spots_available'] <= plot_from_min]
...
def plot_gb_time_series(df, ts_name, gb_name, value_name, figsize=(40,10), title=None):
    ...
    for key, grp in df.groupby([gb_name]):
        ax = grp.plot(ax=ax, linestyle='', ms=12, x=ts_name, y=value_name, label=key, marker='o')
